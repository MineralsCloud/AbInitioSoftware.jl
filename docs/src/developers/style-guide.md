...
!!! info
    The style guide is always a work in progress, and not all AbInitioSoftware code
    follows the rules. When modifying AbInitioSoftware, please fix the style violations
...
AbInitioSoftware uses [JuliaFormatter](https://github.com/domluna/JuliaFormatter.jl) as
an auto-formatting tool.

We use the options contained in [`.JuliaFormatter.toml`](https://github.com/MineralsCloud/AbInitioSoftware.jl/blob/main/.JuliaFormatter.toml).

To format your code, `cd` to the AbInitioSoftware directory, then run:
...
!!! info
    A continuous integration check verifies that all PRs made to AbInitioSoftware have
