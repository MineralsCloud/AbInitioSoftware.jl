...
[AbInitioSoftware](https://github.com/MineralsCloud/AbInitioSoftware.jl).
...
2. Run the following commands and wait for them to finish:
   ...
   julia> Pkg.add("AbInitioSoftware")
   ```

3. Run

   ```julia-repl
   julia> using AbInitioSoftware
   ```
...
version of AbInitioSoftware, type

```@repl
using Pkg
Pkg.update()
pkg"add https://github.com/MineralsCloud/AbInitioSoftware.jl"
```

in the second step above.

## Update AbInitioSoftware
...
our [GitHub repository](https://github.com/MineralsCloud/AbInitioSoftware.jl)
for new releases.
Once we release a new version, you can update AbInitioSoftware by typing

```@repl
using Pkg
Pkg.update("AbInitioSoftware")
Pkg.gc()
```

in the Julia REPL.

## Uninstall and reinstall AbInitioSoftware
...
1. To uninstall, in a Julia session, run
   ...
   julia> Pkg.rm("AbInitioSoftware")

   julia> Pkg.gc()
   ```

2. Press `ctrl+d` to quit the current session. Start a new Julia session and
   reinstall AbInitioSoftware.
